...
const servicio = Number(localStorage.getItem("servicio"));
...
(async function () {
    try {
        ...
        // si es de manufactura (no == 1) elimina manufactura (21)
        secciones = seccionRec.records.filter((sec) => {
            if (servicio == 1) return sec.seccion != 20;
            if (servicio !== 1) return sec.seccion != 21;
            return true; // En caso de que servicio tenga otro valor, no se filtra nada.
        });

        // secciones = seccionRec.records;

        console.log("Secciones filtradas:", secciones);
